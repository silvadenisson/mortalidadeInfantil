...
# 3. merge das bases de dados - Agregacao dia   #
# --------------------------------------------- #
...
mort_infantil$dia <- substr(mort_infantil$DTOBITO, 1, 2)

# agregando por Municipio e Ano
mort_infantil <- aggregate(Total_mort_inf ~ Ano + mes + dia + CODMUNRES, data = mort_infantil, sum)
...
mort_infantil1 <- mort_infantil1[, c(6, 1:5)]
mort_infantil2 <- mort_infantil2[, c(1, 6, 2:5)]
...
nascido0005$dia <- substr(nascido0005$DTNASC, 1, 2)
nascido0005 <- aggregate(Total_NascidoVivo ~ Ano + mes + dia + CODMUNRES, data = nascido0005, sum)
...
nascido0614$dia <- substr(nascido0614$DTNASC, 1, 2)
nascido0614 <- aggregate(Total_NascidoVivo ~ Ano + mes + dia + CODMUNRES, data = nascido0614, sum)
nascido0614 <- merge(nascido0614, mun, by.x = "CODMUNRES", by.y = "IBGE6", all.x = T)
names(nascido0614)[1] <- "IBGE6"

nascido0005 <- nascido0005[, c(6, 1:5, 7:15)]
nascido0614 <- nascido0614[, c(1, 6, 2:5, 7:15)]
...
base <- merge(nascidosVivos, mort_infantil, by = c("Ano", "mes", "dia","IBGE6" , "IBGE7"), all = T)
...
write.csv2(base, "indicadores_mortalidade_infantil_dia.csv")
saveRDS(base, "indicadores_mortalidade_infantil_dia.Rda")
